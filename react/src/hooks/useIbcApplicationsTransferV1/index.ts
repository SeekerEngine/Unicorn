...
export default function useIbcApplicationsTransferV1() {
  ...
  const QueryDenomTraces = (query: any, options: any, perPage: number) => {
    ...
    return useInfiniteQuery([key], ({pageParam = 1}: { pageParam?: number}) => {
      ...
      getNextPageParam: (lastPage, allPages) => { if (Number(Number(lastPage.pagination?.total ?? 0)) >((lastPage.pageParam ?? 0) * perPage)) {return lastPage.pageParam+1 } else {return undefined}},
